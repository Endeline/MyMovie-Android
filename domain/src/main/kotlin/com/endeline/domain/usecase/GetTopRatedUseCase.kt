...
class GetTopRatedUseCase : ObservableUseCaseWithTwoParams<ProductType, ProductsUiModel> {
    ...
    override fun invoke(productType: ProductType): Observable<ProductsUiModel> =
        productService.getTopRated(productType.name).map { it.toUiModel(productType) }
}
